...
public class Ex1_p529 {
	...
	public static void main(String[] args) {
		...
		// 1. Math 클래스 : 수학 계산에 사용할 수 있는 메소드 제공
			// 1. ceil(값)	: 올림 함수 	[소수 첫째자리]
			// 2. floor(값)	: 버림 함수 	[소수 첫째자리]
			// 3. round(값)	: 반올림 함수	[소수 첫째자리] / String.format("%.소수자리수f",값)
			// 3. max(값1,값2): 최대값
			// 4. min(값1,값2): 최소값
			...
			double v5 = Math.round(5.367);	System.out.println("v5: "+v5);
		
			// 소수 셋째자리에서 반올림하여 두자리까지 표시
			double 	value = 12.3456;
			double 	temp1 = value*100;			// 1234.56
			long 	temp2 = Math.round(temp1);	// 1235
			double 	v6 	  = temp2/100.0;		// 12.35		
			System.out.println("v6: "+v6);
			// 소수 네번째자리에서 반올림하여 세자리까지 표시
			System.out.println("반올림: "+((Math.round(12.3456*1000))/1000.0));
			// 문자열 소수 자리수 표현
			System.out.println(String.format("%.2f",3.14159));
			
		// 2. 
			// 킄래스  : 서로 다른 자료형/타입 한곳[객체]에 저장
			// 배열/리스트 : 동일한 자료형/타입의 데이터를 여러개 저장
			
			// int형 변수 6개를 저장할 수 있는 배열	 [ 고정길이, 메소드제공x ]
			int[] selectNumber = new int[6];	
				//vs
			// int형 변수 6개를 저장할 수 있는 리스트[ 가변길이, 메소드제공o ]
			ArrayList<Integer> selectNumber2 = new ArrayList<>(6);
				// 포장클래스 : 기본자료형 -> 클래스 [ * 특정 API 클래스 중 기본자료형 사용불가 ]
			...
			System.out.println();
			// 배열에 정렬
				// 1. Arrays.sort(배열) 		: 해당 배열내 데이터를 오름차순으로 정렬
